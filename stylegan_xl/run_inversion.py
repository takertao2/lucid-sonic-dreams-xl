...
def space_regularizer_loss(
    ...
    if not G_original.c_dim:
        ...
        c_samples = F.one_hot(
            torch.randint(G_original.c_dim, (num_of_sampled_latents, )),
            G_original.c_dim)
    ...
    territory_indicator_ws = [
        get_morphed_w_code(w_code.unsqueeze(0), w_batch)
        for w_code in w_samples
    ]

    for w_code in territory_indicator_ws:
        new_img = G_pti.synthesis(w_code, noise_mode='none', force_fp32=True)
        with torch.no_grad():
            old_img = G_original.synthesis(w_code,
                                           noise_mode='none',
                                           force_fp32=True)
...
def pivotal_tuning(
    ...
    for step in range(num_steps):
        ...
        synth_images = G_pti.synthesis(w_pivot[0].repeat(1, G.num_ws, 1),
                                       noise_mode=noise_mode)

        # track images
        synth_images = (synth_images + 1) * (255 / 2)
        synth_images_np = synth_images.clone().detach().permute(
            0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
...
def project(
    ...
    *,
    ...
    z_samples = torch.from_numpy(
        np.random.RandomState(123).randn(w_avg_samples, G.z_dim)).to(device)

    # get class probas by classifier
    if not G.c_dim:
        c_samples = None
    else:
        classifier = timm.create_model('deit_base_distilled_patch16_224',
                                       pretrained=True).eval().to(device)
        cls_target = F.interpolate(
            (target.to(device).to(torch.float32) / 127.5 - 1)[None], 224)
        logits = classifier(cls_target).softmax(1)
        classes = torch.multinomial(logits, w_avg_samples,
                                    replacement=True).squeeze()
        print(
            f'Main class: {logits.argmax(1).item()}, confidence: {logits.max().item():.4f}'
        )
    ...
    for step in range(num_steps):
        ...
        # Synth images from opt_w.
        synth_images = G.synthesis(w_opt[0].repeat(1, G.num_ws, 1),
                                   noise_mode=noise_mode)
...
@click.option('--outdir',
              ...
              required=True,
              metavar='DIR')
@click.option('--inv-steps',
              help='Number of inversion steps',
              type=int,
              default=1000,
              show_default=True)
@click.option('--w-init',
              help='path to inital latent',
              type=str,
              default='',
              show_default=True)
@click.option('--run-pti', help='run pivotal tuning', is_flag=True)
@click.option('--pti-steps',
              help='Number of pti steps',
              type=int,
              default=350,
              show_default=True)
def run_projection(
    ...
    if save_video:
        video = imageio.get_writer(f'{outdir}/proj.mp4',
                                   mode='I',
                                   fps=60,
                                   codec='libx264',
                                   bitrate='16M')
