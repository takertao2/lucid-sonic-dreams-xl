...
class PPLSampler(torch.nn.Module):
    ...
    def forward(self, c):
        ...
        t = torch.rand([c.shape[0]],
                       device=c.device) * (1 if self.sampling == 'full' else 0)
        z0, z1 = torch.randn([c.shape[0] * 2, self.G.z_dim],
                             device=c.device).chunk(2)

        # Interpolate in W or Z.
        if self.space == 'w':
            w0, w1 = self.G.mapping(z=torch.cat([z0, z1]),
                                    c=torch.cat([c, c])).chunk(2)
            wt0 = w0.lerp(w1, t.unsqueeze(1).unsqueeze(2))
            wt1 = w0.lerp(w1, t.unsqueeze(1).unsqueeze(2) + self.epsilon)
        else:  # space == 'z'
            zt0 = slerp(z0, z1, t.unsqueeze(1))
            zt1 = slerp(z0, z1, t.unsqueeze(1) + self.epsilon)
            wt0, wt1 = self.G.mapping(z=torch.cat([zt0, zt1]),
                                      c=torch.cat([c, c])).chunk(2)
        ...
        # Generate images.
        img = self.G.synthesis(ws=torch.cat([wt0, wt1]),
                               noise_mode='const',
                               force_fp32=True,
                               **self.G_kwargs)
        ...
        if factor > 1:
            img = img.reshape([
                -1, img.shape[1], img.shape[2] // factor, factor,
                img.shape[3] // factor, factor
            ]).mean([3, 5])
        ...
        lpips_t0, lpips_t1 = self.vgg16(img,
                                        resize_images=False,
                                        return_lpips=True).chunk(2)
...
def compute_ppl(opts, num_samples, epsilon, space, sampling, crop, batch_size):
    ...
    vgg16 = metric_utils.get_feature_detector(vgg16_url,
                                              num_gpus=opts.num_gpus,
                                              rank=opts.rank,
                                              verbose=opts.progress.verbose)

    # Setup sampler and labels.
    sampler = PPLSampler(G=opts.G,
