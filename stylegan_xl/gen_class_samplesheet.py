...
@click.option('--classes',
              type=parse_range,
              help='List of classes (e.g., \'0,1,4-6\')',
              required=True)
@click.option('--samples-per-class',
              help='Samples per class.',
              type=int,
              default=4)
@click.option('--grid-width',
              help='Total width of image grid',
              type=int,
              default=32)
@click.option('--batch-gpu',
              help='Samples per pass, adapt to fit on GPU',
              type=int,
              default=32)
@click.option('--outdir',
...
def generate_samplesheet(
    ...
    with dnnlib.util.open_url(network_pkl) as f:
        G = legacy.load_network_pkl(f)['G_ema'].to(device).requires_grad_(
            False)
    ...
    images = gen_utils.create_image_grid(np.concatenate(images),
                                         grid_size=(grid_width, None))
