...
class GlfwWindow:  # pylint: disable=too-many-public-methods
    ...
    def __init__(self,
        ...
        self._glfw_window = glfw.create_window(width=window_width,
    ...
    def set_window_size(self, width, height):
        ...
        glfw.set_window_size(self._glfw_window, width,
                             max(height - self.title_bar_height, 0))
    ...
    def should_close(self):
        return glfw.window_should_close(
            self._glfw_window) or (self._close_on_esc and self._esc_pressed)
    ...
    def begin_frame(self):
        ...
        if self._frame_start_time is not None and self._fps_limit is not None:
            delay = self._frame_start_time - time.perf_counter(
            ) + 1 / self._fps_limit
            if delay > 0:
                time.sleep(delay)
        ...
        gl.glBlendFunc(gl.GL_ONE,
                       gl.GL_ONE_MINUS_SRC_ALPHA)  # Pre-multiplied alpha.
