...
def init_egl():
    assert os.environ[
        'PYOPENGL_PLATFORM'] == 'egl'  # Must be set before importing OpenGL.
    ...
    # Choose config.
    config_attribs = [
        egl.EGL_RENDERABLE_TYPE, egl.EGL_OPENGL_BIT, egl.EGL_SURFACE_TYPE,
        egl.EGL_PBUFFER_BIT, egl.EGL_NONE
    ]
...
_texture_formats = {
    ('uint8', 1):
    dnnlib.EasyDict(type=gl.GL_UNSIGNED_BYTE,
    ...
    ('float32', 3):
    dnnlib.EasyDict(type=gl.GL_FLOAT,
                    format=gl.GL_RGB,
...
def read_pixels(width, height, *, pos=0, dtype='uint8', channels=3):
    ...
    gl.glReadPixels(int(np.round(pos[0])), int(np.round(pos[1])), width,
                    height, fmt.format, fmt.type, image)
    gl.glPopClientAttrib()
...
class Texture:
    ...
    def __init__(self,
        ...
        assert self.is_compatible(width=width,
                                  height=height,
                                  channels=channels,
                                  dtype=dtype)
    ...
    def update(self, image):
        ...
        with self.bind():
            ...
            gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, fmt.internalformat,
                            self.width, self.height, 0, fmt.format, fmt.type,
                            image)
            if self.mipmap:
    ...
    def is_compatible(self,
        ...
        if image is not None:
            ...
            if not self.is_compatible(
                    width=iw, height=ih, channels=ic, dtype=image.dtype):
                return False
        ...
        with self.bind():
            ...
            if self.texture is not None:
                ...
                gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER,
                                          ...
                                          gl.GL_TEXTURE_2D, self.texture.gl_id,
                                          0)
            else:
                fmt = get_texture_format(self.dtype, self.channels)
...
def draw_shape(vertices,
    ...
    alpha = np.clip(np.broadcast_to(np.asarray(alpha, dtype='float32'), []), 0,
                    1)
...
def draw_rect(*,
    ...
    size = size if size is not None else pos2 - pos if pos2 is not None else np.array(
        [1, 1], dtype='float32')
    pos = pos - size * align
    if rint:
        pos = np.rint(pos)
