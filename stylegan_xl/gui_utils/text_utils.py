...
def _get_array_priv(string: str,
                    *,
                    size: int = 32,
    ...
    while True:
        if dropshadow_radius is not None:
            ...
            array = _get_array_impl_dropshadow(string,
                                               size=cur_size,
        ...
        if (max_width is None or width <= max_width) and (
                max_height is None or height <= max_height) or (cur_size
                                                                <= min_size):
            break
...
def _get_array_impl(string,
    ...
    lines = [
        np.array(line, dtype=np.uint8).reshape([line.size[1], line.size[0]])
        for line in lines
    ]
    width = max(line.shape[1] for line in lines)
    lines = [
        np.pad(line, ((0, 0), (0, width - line.shape[1])), mode='constant')
        for line in lines
    ]
    line_spacing = line_pad if line_pad is not None else size // 2
    lines = [
        np.pad(line, ((0, line_spacing), (0, 0)), mode='constant')
        for line in lines[:-1]
    ] + lines[-1:]
    ...
    if outline > 0:
        mask = np.pad(mask,
                      int(np.ceil(outline * outline_pad)),
                      mode='constant',
                      constant_values=0)
...
@functools.lru_cache(maxsize=10000)
def _get_array_impl_dropshadow(string,
                               *,
                               font=None,
                               size=32,
                               radius: int,
    ...
    mask = np.pad(mask,
                  2 * radius + max(abs(offset_x), abs(offset_y)),
                  mode='constant',
                  constant_values=0)
    alpha = mask.astype(np.float32) / 255
...
def get_texture(string, bilinear=True, mipmap=True, **kwargs):
    return gl_utils.Texture(image=get_array(string, **kwargs),
                            bilinear=bilinear,
                            mipmap=mipmap)
