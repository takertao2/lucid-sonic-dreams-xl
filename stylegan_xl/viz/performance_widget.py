...
class PerformanceWidget:
    ...
    def __call__(self, show=True):
        ...
        if 'render_time' in viz.result:
            self.render_times = self.render_times[1:] + [
                viz.result.render_time
            ]
        ...
        if show:
            ...
            with imgui_utils.item_width(viz.font_size * 8):
                imgui.plot_lines('##gui_times',
                                 array.array('f', self.gui_times),
                                 scale_min=0)
            ...
            imgui.same_line(imgui.get_content_region_max()[0] - 1 -
                            viz.button_w * 2 - viz.spacing)
            _clicked, self.use_vsync = imgui.checkbox('Vertical sync',
                                                      self.use_vsync)
        ...
        if show:
            ...
            imgui.same_line(viz.label_w)
            with imgui_utils.item_width(viz.font_size * 8):
                imgui.plot_lines('##render_times',
                                 array.array('f', self.render_times),
                                 scale_min=0)
            imgui.same_line(viz.label_w + viz.font_size * 9)
            ...
            imgui.text(f'{1/t:.1f} FPS' if t > 0 else 'N/A')
            imgui.same_line(viz.label_w + viz.font_size * 18 + viz.spacing * 3)
            _clicked, self.is_async = imgui.checkbox('Separate process',
                                                     self.is_async)
            imgui.same_line(imgui.get_content_region_max()[0] - 1 -
                            viz.button_w * 2 - viz.spacing)
            _clicked, self.force_fp32 = imgui.checkbox('Force FP32',
                                                       self.force_fp32)
