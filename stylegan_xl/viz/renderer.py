...
def _construct_affine_bandlimit_filter(mat,
                                       a=3,
                                       amax=16,
                                       aflt=64,
                                       up=4,
...
class Renderer:
    ...
    def _render_impl(
        ...
        res.num_ws = G.num_ws
        res.has_noise = any('noise_const' in name
                            for name, _buf in G.synthesis.named_buffers())
        res.has_input_transform = (hasattr(G.synthesis, 'input')
                                   and hasattr(G.synthesis.input, 'transform'))
        ...
        # Calculate final W.
        w = torch.stack([all_ws[seed] * weight
                         for seed, weight in w0_seeds]).sum(dim=0,
                                                            keepdim=True)
        stylemix_idx = [idx for idx in stylemix_idx if 0 <= idx < G.num_ws]
        if len(stylemix_idx) > 0:
            w[:, stylemix_idx] = all_ws[stylemix_seed][np.newaxis,
                                                       stylemix_idx]
        w += w_avg

        # Run synthesis network.
        synthesis_kwargs = dnnlib.EasyDict(noise_mode=noise_mode,
                                           force_fp32=force_fp32)
        torch.manual_seed(random_seed)
        out, layers = self.run_synthesis_net(G.synthesis,
                                             w,
                                             capture_layer=layer_name,
                                             **synthesis_kwargs)
        ...
        if cache_key not in self._net_layers:
            if layer_name is not None:
                torch.manual_seed(random_seed)
                _out, layers = self.run_synthesis_net(G.synthesis, w,
                                                      **synthesis_kwargs)
        ...
        if untransform and res.has_input_transform:
            out, _mask = _apply_affine_transformation(
                out.to(torch.float32), G.synthesis.input.transform,
        ...
        img = sel
        if img_normalize:
            img = img / img.norm(float('inf'), dim=[1, 2], keepdim=True).clip(
                1e-8, 1e8)
        ...
        if fft_show:
            ...
            sig = sig * torch.kaiser_window(sig.shape[1],
                                            periodic=False,
                                            beta=fft_beta,
                                            device=self._device)[None, :, None]
            sig = sig * torch.kaiser_window(sig.shape[2],
                                            periodic=False,
                                            beta=fft_beta,
                                            device=self._device)[None, None, :]
            fft = torch.fft.fftn(sig, dim=[1, 2]).abs().square().sum(dim=0)
            fft = fft.roll(shifts=[fft.shape[0] // 2, fft.shape[1] // 2],
                           dims=[0, 1])
    ...
    def run_synthesis_net(net,
        ...
        def module_hook(module, _inputs, outputs):
            ...
            for idx, out in enumerate(outputs):
                ...
                layers.append(
                    dnnlib.EasyDict(name=name, shape=shape, dtype=dtype))
