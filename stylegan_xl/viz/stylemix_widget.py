    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            with imgui_utils.item_width(
                    viz.font_size * 8), imgui_utils.grayed_out(num_ws == 0):
            ...
            for idx in range(num_enables):
                imgui.same_line(
                    round(pos0 + (pos1 - pos0) * (idx / (num_enables - 1))))
                if idx == 0:
                    imgui.set_cursor_pos_y(imgui.get_cursor_pos_y() + 3)
                with imgui_utils.grayed_out(num_ws == 0):
                    _clicked, self.enables[idx] = imgui.checkbox(
                        f'##{idx}', self.enables[idx])
            ...
            with imgui_utils.grayed_out(num_ws == 0):
                if imgui_utils.button(
                        'Reset',
                        width=-1,
                        enabled=(self.seed != self.seed_def or self.animate
        ...
        if any(self.enables[:num_ws]):
            viz.args.stylemix_idx = [
                idx for idx, enable in enumerate(self.enables) if enable
            ]
