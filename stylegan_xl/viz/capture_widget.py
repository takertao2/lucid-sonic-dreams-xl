    ...
    def __call__(self, show=True):
        ...
        if show:
            with imgui_utils.grayed_out(self.disabled_time != 0):
                ...
                ) and not imgui.is_item_active() and self.path != '':
                    imgui.set_tooltip(self.path)
                imgui.same_line()
                if imgui_utils.button('Save image',
                                      width=viz.button_w,
                                      enabled=(self.disabled_time == 0
                                               and 'image' in viz.result)):
                    self.dump_image = True
                    self.defer_frames = 2
                    self.disabled_time = 0.5
                imgui.same_line()
                if imgui_utils.button('Save GUI',
                                      width=-1,
                                      enabled=(self.disabled_time == 0)):
                    self.dump_gui = True
                    self.defer_frames = 2
