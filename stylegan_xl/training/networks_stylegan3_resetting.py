...
class MappingNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        self.embed_proj = FullyConnectedLayer(
            self.embed.embedding_dim, self.z_dim,
            activation='lrelu') if self.c_dim > 0 else None

        features = [self.z_dim + (self.z_dim if self.c_dim > 0 else 0)
    ...
    def forward(self,
        ...
        if update_emas:
            ...
            if self.c_dim > 0:
                for i, label in enumerate(c.argmax(1)):
                    self.w_avg[label].copy_(x[i].detach().lerp(
                        self.w_avg[label], self.w_avg_beta))
            else:
        ...
        if truncation_psi != 1:
            if self.c_dim > 0:
                for i, label in enumerate(c.argmax(1)):
                    x[i, :truncation_cutoff] = self.w_avg[label].lerp(
                        x[i, :truncation_cutoff], truncation_psi)
            else:
...
class SynthesisLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.init_upfirdn(in_sampling_rate, out_sampling_rate, in_cutoff,
                          out_cutoff, in_half_width, out_half_width, in_size,
                          out_size)

    def init_upfirdn(self,
                     in_sampling_rate,
                     out_sampling_rate,
                     in_cutoff,
                     out_cutoff,
        ...
        self.tmp_sampling_rate = max(
            self.in_sampling_rate, out_sampling_rate) * (
                1 if self.is_torgb else self.lrelu_upsampling)
        self.in_cutoff = in_cutoff
        self.out_cutoff = out_cutoff
        self.in_half_width = in_half_width
        self.out_half_width = out_half_width
    ...
    def _interp_lrelufilters(self, filt_interp_weight):
        if self.up_filter is not None:
            self.up_filter = self.up_filter4.lerp(self.up_filter16,
                                                  filt_interp_weight)
        if self.down_filter is not None:
            self.down_filter = self.down_filter4.lerp(self.down_filter16,
                                                      filt_interp_weight)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        for idx in range(self.num_layers + 1):
            ...
            use_fp16 = (sampling_rates[idx] * (2**self.num_fp16_res)
                        > self.img_resolution)
    ...
    def get_layer_specs(self):
        ...
        cutoffs = self.first_cutoff * (last_cutoff /
                                       self.first_cutoff)**exponents  # f_c[i]
        stopbands = self.first_stopband * (
            last_stopband / self.first_stopband)**exponents  # f_t[i]
        ...
        channels = np.rint(
            np.minimum((self.channel_base / 2) / cutoffs, self.channel_max))
...
class SuperresGenerator(torch.nn.Module):
    ...
    def __init__(
        ...
        **synthesis_kwargs,
        ...
        assert up_factor in [2, 4, 8,
                             16], "Supported up_factors: [2, 4, 8, 16]"
        ...
        last_stem_layer = getattr(self.synthesis,
                                  self.synthesis.layer_names[-1])
        fparams = self.compute_superres_filterparams(up_factor,
                                                     self.img_resolution,
                                                     last_stem_layer,
                                                     head_layers)
        ...
        for idx in range(head_layers + 1):
            ...
            use_fp16 = (fparams.sampling_rates[idx] * (2**self.num_fp16_res)
                        > self.img_resolution)
            layer = SynthesisLayer(
                ...
                is_critically_sampled=is_critically_sampled,
                use_fp16=use_fp16,
                in_channels=int(fparams.channels[prev]),
                out_channels=int(fparams.channels[idx]),
                in_size=int(fparams.sizes[prev]),
                out_size=int(fparams.sizes[idx]),
                in_sampling_rate=int(fparams.sampling_rates[prev]),
                out_sampling_rate=int(fparams.sampling_rates[idx]),
                in_cutoff=fparams.cutoffs[prev],
                out_cutoff=fparams.cutoffs[idx],
                in_half_width=fparams.half_widths[prev],
                out_half_width=fparams.half_widths[idx],
                conv_kernel=self.conv_kernel,
                use_radial_filters=self.use_radial_filters,
            )
    ...
    def compute_superres_filterparams(self,
                                      up_factor,
                                      img_resolution,
                                      last_stem_layer,
                                      head_layers,
        ...
        last_stopband = last_cutoff * self.last_stopband_rel  # f_{t,N}
        exponents = np.minimum(
            np.arange(head_layers + 1) / (head_layers - num_critical), 1)
        ...
        # set sampling rates
        sampling_rates = np.exp2(
            np.ceil(np.log2(np.minimum(stopbands * 2, img_resolution))))
