...
def training_loop(
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        if ckpt_pkl is not None:  # Load ticks
            ...
            __AUGMENT_P__ = resume_data['progress'].get(
                'augment_p', torch.tensor(0.)).to(device)
    ...
    iter_dict = {
        'G': 1,
        'D': 1
    }  # change here if you want to do several G/D iterations at once
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z,
            ...
            if phase.name in ['Dmain', 'Dboth', 'Dreg'] and hasattr(
                    phase.module, 'feature_networks'):
        ...
        if (not done) and (cur_tick != 0) and (cur_nimg < tick_start_nimg +
                                               kimg_per_tick * 1000):
            continue
        ...
        if (rank
                == 0) and (restart_every
                           > 0) and (network_snapshot_ticks is not None) and (
