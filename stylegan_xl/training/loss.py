...
class ProjectedGANLoss(Loss):
    ...
    def run_G(self, z, c, update_emas=False):
        ...
        img = self.G.synthesis(
            ws, update_emas=False)  # enabling emas leads to collapse with PG
        return img, ws
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gain,
        ...
        if do_Gmain:
            ...
            if self.train_head_only:
                ...
                for name in self.G.synthesis.layer_names:
                    getattr(
                        self.G.synthesis,
                        name).requires_grad_(name in self.G.head_layer_names)
            ...
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                if self.cls_weight:
                    ...
                    guidance_loss = self.cls_guidance_loss(
                        self.classifier(gen_img), gen_c.argmax(1))
                    loss_Gmain += self.cls_weight * guidance_loss
                    training_stats.report('Loss/G/guidance_loss',
                                          guidance_loss)
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                gen_logits = self.run_D(gen_img, gen_c, blur_sigma=blur_sigma)
                loss_Dgen = sum([(F.relu(torch.ones_like(l) + l)).mean()
                                 for l in gen_logits])
                gen_logits = torch.cat(gen_logits)
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                loss_Dreal = sum([(F.relu(torch.ones_like(l) - l)).mean()
